...
export default function VacancyDetails({ lang = 'en' }) {
  const { employees } = useEmployee();
  const { t } = useLanguage();

  // Zones as per Excel
  const zonesData = [
    { name: 'Central-Lucknow', s: 8, keywords: ['CENTRAL-LUCKNOW', 'CENTRAL, LUCKNOW'] },
    { name: 'South-East, Prayagraj', s: 7, keywords: ['SOUTH-EAST, PRAYAGRAJ', 'SOUTH-EAST'] },
    { name: 'West-Meerut', s: 9, keywords: ['WEST-MEERUT'] },
    { name: 'South-West, Agra', s: 7, keywords: ['SOUTH-WEST, AGRA', 'SOUTH-WEST'] },
    { name: 'North-East, Gorakhpur', s: 5, keywords: ['NORTH-EAST, GORAKHPUR', 'NORTH-EAST'] },
    { name: 'South-Central, Jhansi', s: 4, keywords: ['SOUTH-CENTRAL, JHANSI', 'SOUTH-CENTRAL'] },
    { name: 'C&C, Lucknow', s: 3, keywords: ['C&C'] },
    { name: 'D&P, Lucknow', s: 3, keywords: ['D&P'] },
    { name: '765 kV, Design Unit, Lucknow', s: 2, keywords: ['765', 'DESIGN UNIT'] },
    { name: 'Director (Commercial & Planning), Lucknow', s: 2, keywords: ['COMMERCIAL', 'C&P'] },
    { name: 'Director (PM&A), Lucknow', s: 3, keywords: ['PM&A'] },
    { name: 'Director (W&P), Lucknow', s: 1, keywords: ['W&P'] },
    { name: 'Director (Operation), Lucknow', s: 0, keywords: ['OPERATION'] },
    { name: 'Circle to be Created', s: 3, keywords: ['CREATE'] },
    { name: 'UP SLDC LTD.', s: 6, keywords: ['SLDC'] },
    // Civil
    { name: 'Civil-I, Lucknow', s: 4, keywords: ['CIVIL-I, LUCKNOW', 'CIVIL-I,LUCKNOW'] },
    { name: 'Civil-II, Lucknow', s: 3, keywords: ['CIVIL-II, LUCKNOW', 'CIVIL-II,LUCKNOW'] }
  ];

  const targetDesigs = ['Superintending Engineer/Joint Secretary (E&M)', 'Superintending Engineer (E&M)', 'SE (E&M)', 'Superintending Engineer', 'Superintending Engineer (Civil)', 'SE (Civil)'];
  ...
  const workingEmps = employees.filter(emp => {
    ...
    return targetDesigs.some(d => empDesig.includes(d));
  });
      ...
      if (unaccounted !== 0) {
        if (unaccounted < 0) {
          ...
          if (!isFieldZone && z.name !== 'UP SLDC LTD.') {
            ...
            vRemarks += unaccounted; // Field Default put missing vacancies in Remarks
          }
        }
      }
    }
      ...
      <div className="bg-white p-0 shadow-sm rounded border border-dark">
        <div className="text-center pt-3 pb-2 border-bottom border-dark position-relative">
          <h3 className="mb-0 fw-bold text-decoration-underline" style={{ fontFamily: 'Times New Roman', letterSpacing: '0.5px' }}>
            {t('SE (E&M/Civil) Vacancy Details')}
          </h3>
        </div>
        ...
        <div className="table-responsive shadow-sm rounded border border-dark mb-4">
          <table className="table table-hover table-bordered table-sm align-middle text-center mb-0 border-dark" style={{ fontFamily: 'Arial, sans-serif', fontSize: '0.95rem' }}>
            <thead style={{ backgroundColor: '#1e3a8a', color: '#ffffff', borderBottom: '3px solid #000' }}>
              ...
              <tr>
                <th style={{ width: '9%' }} dangerouslySetInnerHTML={{ __html: t('Regular Circle').replace(' ', '<br/>') }}></th>
                <th style={{ width: '9%' }} dangerouslySetInnerHTML={{ __html: t('T&C Circle').replace(' ', '<br/>') }}></th>
